...
var (
	...
	proxyCmd = &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			// check if SDS UDS path and token path exist, if both exist, requests key/cert using SDS
			// instead of secret mount.
			// TODO(quanlin): move getting both path from env variable.
			...
			trustworthyJWTPath := "/var/run/secrets/kubernetes.io/serviceaccount/token"
			...
			if controlPlaneBootstrap {
				if templateFile != "" && proxyConfig.CustomConfigFile == "" {
					...
					log.Infof("*******sdsEnabled %+v", sdsEnabled)
					log.Infof("******token path is %q", sdsTokenPath)
					if sdsEnabled {
