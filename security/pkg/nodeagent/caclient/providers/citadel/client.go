...
// NewCitadelClient create an CA client for Citadel.
func NewCitadelClient(conn *grpc.ClientConn) caClientInterface.Client {
	log.Infof("*****NewCitadelClient")
	return &citadelClient{
		client: capb.NewIstioCertificateServiceClient(conn),
	}
}

func (cl *citadelClient) CSRSign(ctx context.Context, csrPEM []byte, token string,
	certValidTTLInSec int64) ([]string /*PEM-encoded certificate chain*/, error) {
	log.Infof("*****CitadelClient CSRSign using token %q", token)
	...
	log.Infof("******CitadelClient received cert chain %+v", resp.CertChain)
