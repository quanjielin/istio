...
var (
	...
	fakeToken           = "faketoken"
	fakeTokenExpireTime = time.Date(2050, 12, 31, 1, 0, 0, 0, time.UTC)
)

type mockIAMServer struct{}

func (*mockIAMServer) GenerateIdentityBindingAccessToken(ctx context.Context, in *iam.GenerateIdentityBindingAccessTokenRequest) (*iam.GenerateIdentityBindingAccessTokenResponse, error) {
	et, _ := ptypes.TimestampProto(fakeTokenExpireTime)
...
func TestIAMClientPlugin(t *testing.T) {
	...
	if expireTime != fakeTokenExpireTime {
		t.Errorf("resp expireTime: got %+v, expected %q", expireTime, fakeTokenExpireTime)
	}
}
