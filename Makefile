...
generate_yaml: $(HELM) $(HOME)/.helm helm-repo-add
	...
	cat install/kubernetes/namespace.yaml > install/kubernetes/istio-citadel.yaml
	$(HELM) template --set global.tag=${TAG} \
		--name=istio \
		--namespace=istio-system \
		--set global.hub=${HUB} \
		...
		--set istio_cni.enabled=${ENABLE_ISTIO_CNI} \
		${EXTRA_HELM_SETTINGS} \
		--values install/kubernetes/helm/istio/values-istio-citadel.yaml \
		install/kubernetes/helm/istio >> install/kubernetes/istio-citadel.yaml
