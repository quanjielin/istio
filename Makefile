...
generate_google_e2e_yaml: $(HELM) $(HOME)/.helm istio-init.yaml
	cat install/kubernetes/namespace.yaml > install/kubernetes/istio-googleca1.yaml
	cat install/kubernetes/helm/istio-init/files/crd-* >> install/kubernetes/istio-googleca1.yaml
	$(HELM) template \
		--name=istio \
		--namespace=istio-system \
		--set global.tag=${TAG} \
		...
		${EXTRA_HELM_SETTINGS} \
		--values install/kubernetes/helm/istio/test-values/values-e2e.yaml \
		--values install/kubernetes/helm/istio/example-values/values-istio-googleca.yaml \
		install/kubernetes/helm/istio >> install/kubernetes/istio-googleca1.yaml
