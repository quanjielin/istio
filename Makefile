...
generate_yaml: $(HELM) $(HOME)/.helm helm-repo-add
	...
	cat install/kubernetes/namespace.yaml > install/kubernetes/istio-googleeap-prod.yaml
	cat install/kubernetes/helm/istio-init/files/crd-* >> install/kubernetes/istio-googleeap-prod.yaml
	$(HELM) template --set global.tag=${TAG} \
		--name=istio \
		--namespace=istio-system \
		--set global.hub=${HUB} \
